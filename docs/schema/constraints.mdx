...
| span        | datetime, date       | A time span from an anchored datetime |
...
### Span
The span constraint is used to define a time span from an anchored datetime, for example, 30 days after a given date. It has three properties:
- **type**: The value needs to be "span".
- **anchor**: The anchored datetime. It is either a string formated as `YYYY-MM-DD HH:MM:SS` or value `<?now?>`. When it's missing, the current datetime is used.
- **leap**: The time leap from the anchored datetime. It consists of the following sub-properties and a none zero value for one of the sub-properties is required:
    - **years**: Number of years from the anchor, value ranges from -10 and 10
    - **months**: Number of months from the anchor, value ranges from -12 and 12
    - **days**: Number of days from the anchor, value ranges from -31 and 31
    - **hours**: Number of hours from the anchor, value ranges from -24 and 24
    - **minutes**: Number of minutes from the anchor, value ranges from -60 and 60
    - **seconds**: Number of seconds from the anchor, value ranges from -60 and 60

```yaml title="Create a date in past 30 days from today"
    properties:
      - name: "in_30_days"
        type: "date"
        constraints:
